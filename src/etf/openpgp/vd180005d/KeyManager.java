...
import etf.openpgp.vd180005d.entities.*;
import etf.openpgp.vd180005d.keymaterial.*;
...
public class KeyManager {
    private static final String PUBLIC_KEY_RING_FILE_PATH = "public_key_ring.gpg" , SECRET_KEY_RING_FILE_PATH = "secret_key_ring.gpg";
     ...
     * @throws PGPException
     */
    public static final void generateKeys(User user, KeyMaterial keyMaterial, SubkeyMaterial subkeyMaterial) throws NoSuchAlgorithmException, NoSuchProviderException, PGPException {
    ...
    private static PGPKeyRingGenerator createKeyRingGenerator(User user, KeyPair keyPair, KeyPair subkeyPair) throws PGPException {
        ...
        PBESecretKeyEncryptor secretKeyEncryptor = (user.getPassphrase().length() == 0 ?
                null : new JcePBESecretKeyEncryptorBuilder(PGPEncryptedData.AES_256, sha1DigestCalculator).setProvider("BC").build(user.getPassphrase().toCharArray()));
